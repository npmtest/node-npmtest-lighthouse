...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-lighthouse/node_modules/lighthouse/lighthouse-cli/chrome-finder.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header medium">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-lighthouse/node_modules/lighthouse/lighthouse-cli/chrome-finder.js</span></h1>
    <h2>
        
        Statements: <span class="metric">67.53% <small>(52 / 77)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="path"><a href="../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-lighthouse/node_modules/lighthouse/lighthouse-cli/</a> &#187; chrome-finder.js</div>
</div>
<div class="body">
...
<span class="fstat-no" title="function not covered" >function darwin() {</span>
<span class="cstat-no" title="statement not covered" >    const suffixes = [</span>
        '/Contents/MacOS/Google Chrome Canary',
        '/Contents/MacOS/Google Chrome'
    ];
<span class="cstat-no" title="statement not covered" >    const LSREGISTER = '/System/Library/Frameworks/CoreServices.framework' +</span>
        '/Versions/A/Frameworks/LaunchServices.framework' +
        '/Versions/A/Support/lsregister';
<span class="cstat-no" title="statement not covered" >    const installations = [];</span>
<span class="cstat-no" title="statement not covered" >    execSync(`${LSREGISTER} -dump` +</span>
        ' | grep -i \'google chrome\\( canary\\)\\?.app$\'' +
        ' | awk \'{$1=""; print $0}\'').toString()
        .split(newLineRegex)
        .forEach((inst) =&gt; {
<span class="cstat-no" title="statement not covered" >        suffixes.forEach(suffix =&gt; {</span>
<span class="cstat-no" title="statement not covered" >            const execPath = path.join(inst.trim(), suffix);</span>
<span class="cstat-no" title="statement not covered" >            if (canAccess(execPath)) {</span>
<span class="cstat-no" title="statement not covered" >                installations.push(execPath);</span>
            }
        });
    });
<span class="cstat-no" title="statement not covered" >    const priorities = [{</span>
...
<span class="cstat-no" title="statement not covered" >    return sort(installations, priorities);</span>
}
...
function linux() {
    let installations = [];
    // 1. Look into LIGHTHOUSE_CHROMIUM_PATH env variable
    <span class="missing-if-branch" title="if path not taken" >I</span>if (canAccess(process.env.LIGHTHOUSE_CHROMIUM_PATH)) {
<span class="cstat-no" title="statement not covered" >        installations.push(process.env.LIGHTHOUSE_CHROMIUM_PATH);</span>
    }
    // 2. Look into the directories where .desktop are saved on gnome based distro's
    ...
    executables.forEach((executable) =&gt; {
        try {
            ...
            <span class="missing-if-branch" title="else path not taken" >E</span>if (canAccess(chromePath)) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!installations.length) {
<span class="cstat-no" title="statement not covered" >        throw new Error('The environment variable LIGHTHOUSE_CHROMIUM_PATH must be set to ' +</span>
...
exports.linux = linux;
<span class="fstat-no" title="function not covered" >function win32() {</span>
<span class="cstat-no" title="statement not covered" >    const installations = [];</span>
<span class="cstat-no" title="statement not covered" >    const suffixes = [</span>
        '\\Google\\Chrome SxS\\Application\\chrome.exe',
        '\\Google\\Chrome\\Application\\chrome.exe'
    ];
<span class="cstat-no" title="statement not covered" >    const prefixes = [</span>
        process.env.LOCALAPPDATA,
        process.env.PROGRAMFILES,
        process.env['PROGRAMFILES(X86)']
    ];
<span class="cstat-no" title="statement not covered" >    if (canAccess(process.env.LIGHTHOUSE_CHROMIUM_PATH)) {</span>
<span class="cstat-no" title="statement not covered" >        installations.push(process.env.LIGHTHOUSE_CHROMIUM_PATH);</span>
    }
<span class="cstat-no" title="statement not covered" >    prefixes.forEach(prefix =&gt; <span class="cstat-no" title="statement not covered" >suffixes.forEach(suffix =&gt; {</span></span>
<span class="cstat-no" title="statement not covered" >        const chromePath = path.join(prefix, suffix);</span>
<span class="cstat-no" title="statement not covered" >        if (canAccess(chromePath)) {</span>
<span class="cstat-no" title="statement not covered" >            installations.push(chromePath);</span>
        }
    }));
<span class="cstat-no" title="statement not covered" >    return installations;</span>
}
exports.win32 = win32;
...
<span class="cstat-no" title="statement not covered" >        return [inst, defaultPriority];</span>
    })
        .sort((a, b) =&gt; b[1] - a[1])
        .map(pair =&gt; pair[0]);
}
...
function findChromeExecutables(folder) {
    ...
    let installations = [];
    <span class="missing-if-branch" title="else path not taken" >E</span>if (canAccess(folder)) {
